...
function addItem(equipment: Record<string, Item>, key: string, item: Item): void {
...
function addChoiceItems(equipment: Record<string, Item>, key: string, choice: ChoiceItem): void {
	choice.items.forEach((item, index) => {
		addItem(equipment, choice.keys?.[index] ?? item.name ?? key, item);
	});
}

function addChoiceOption(equipment: Record<string, Item>, key: string, option: ChoiceOption): void {
	if ("grants" in option) {
...
function calculateHitPoints(
	...
	if (hpMode === "manual") {
		...
		return Math.max(
			1,
			levelOneHp + manualLevels.reduce((total, value) => total + value + conModifier, 0),
		);
	}
...
export function createCharacter(input: CreateCharacterInput): Character {
	...
	const skills = Object.fromEntries(
		Object.entries(defaultSkills).map(([skillName, skill]) => [skillName, { ...skill }]),
	...
	if (classStartingInfo) {
		...
		for (const proficiency of classStartingInfo.armorProficiencies) {
			proficiencies.armor.add(proficiency as Parameters<typeof proficiencies.armor.add>[0]);
		}
		for (const proficiency of classStartingInfo.weaponProficiencies) {
			proficiencies.weapons.add(proficiency as Parameters<typeof proficiencies.weapons.add>[0]);
		}
		for (const proficiency of classStartingInfo.toolProficiencies) proficiencies.tools.add(proficiency);

		for (const [key, choice] of Object.entries(classStartingInfo.startingEquipment.automatic)) {
			addChoiceItems(equipment, key, choice);
		}

		classStartingInfo.startingEquipment.choices.forEach((choiceGroup, index) => {
			const selectedKey = input.classEquipmentSelections?.[index];
			if (!selectedKey) return;
			const selectedOption = selectedKey ? choiceGroup.from[selectedKey] : undefined;
			if (selectedOption) {
	...
	const hp = calculateHitPoints(classDisplayName, level, baseStats, input.hpMode, input.manualHpByLevel);
	proficiencies.languages = new Set(resolved.languages ? [...resolved.languages] : []);
	...
	return {
		...
		level,
		baseStats,
		proficiencyBonus: proficiencyBonus[level as keyof typeof proficiencyBonus] ?? 2,
		inspiration: false,
		savingThrows,
		skills,
