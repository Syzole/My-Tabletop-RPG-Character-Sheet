...
export function applyPatch(species: Species, patch: SpeciesPatch): Species {
	...
	if (patch.features) {
		for (const [name, partial] of Object.entries(patch.features)) {
			...
			} else if (partial.name && partial.type && partial.description && partial.source) {
...
export function mergeSubspecies(parent: Species, sub: Subspecies): Species {
	...
	if (sub.languages) out.languages = new Set([...out.languages, ...sub.languages]);
	...
	if (sub.choices?.length) out.choices = [...(out.choices ?? []), ...sub.choices];
...
export function applySpeciesPicks(species: Species, picks: SpeciesPicks): Species {
...
export function resolveSpecies(
	...
	if (options.subspecies && resolved.subspecies?.[options.subspecies]) {
		resolved = mergeSubspecies(resolved, resolved.subspecies[options.subspecies]);
	}
	if (options.picks) {
