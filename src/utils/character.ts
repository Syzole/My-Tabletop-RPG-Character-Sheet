...
export function calculateAC(character: Character): number {
  ...
  if (character.armor?.maxDexBonus !== undefined) {
    return Math.min(
      ...
      armorClass + character.armor.maxDexBonus,
    );
  }
...
export function calculateAttackBonus(
  ...
  const dexBonus = getModifier(character.baseStats.dexterity);
  const strBonus = getModifier(character.baseStats.strength);
